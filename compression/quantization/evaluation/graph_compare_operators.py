...
# ------ RUN COMPARISON
for bits in quantisation_bits:
    # ffnodynamquant_model = CompressedModel(
    #     model=ffno_model,
    #     compression_technique=lambda model: DynamicQuantization(model, num_bits=bits),
    #     create_replica=True
    # )

    # ffnodynamquant_model = ffnodynamquant_model.to(device)

    # ffnocompare = compare_models(
    #     model1=ffno_model,
    #     model2=ffnodynamquant_model,
    ...
    # fcodanodynamquant_model = CompressedModel(
    #     model=fcodano_model,
    #     compression_technique=lambda model: DynamicQuantization(model, num_bits=bits),
    #     create_replica=True
    # )
    # fcodanodynamquant_model = fcodanodynamquant_model.to(device)

    # fcodano_compare = compare_models(
    #     model1=fcodano_model,
    #     model2=fcodanodynamquant_model,
    ...
    # )
    # results_by_model["fcodano"][bits] = fcodano_compare

    # codanodynamquant_model = CompressedModel(
    #     model=fno_model,
    #     compression_technique=lambda model: DynamicQuantization(model, num_bits=bits)
    #     create_replica=True
    # )
    # codanodynamquant_model = codanodynamquant_model.to(device)

    # codano_compare = compare_models(
    #     model1=codano_model,
    #     model2=codanodynamquant_model,
    #     test_loaders=codanotest_loaders,
    #     data_processor=codanodata_processor,
    #     device=device,
    #     track_performance=True
    # )
    # results_by_model["codano"][bits] = codano_compare

    deepodynamquant_model = CompressedModel(
        model=deeponet_model,
        compression_technique=lambda model: DynamicQuantization(model, num_bits=bits),
        create_replica=True
    )
    deepodynamquant_model = deepodynamquant_model.to(device)

    deepo_compare = compare_models(
        model1=deeponet_model,
        model2=deepodynamquant_model,
        test_loaders=deepotest_loaders,
        data_processor=deepodata_processor,
        device=device,
        track_performance = True
    )
    results_by_model["deeponet"][bits] = deepo_compare

    fnodynamquant_model = CompressedModel(
        model=fno_model,
        compression_technique=lambda model: DynamicQuantization(model, num_bits=bits),
        create_replica=True
    )
    fnodynamquant_model = fnodynamquant_model.to(device)

    fnocompare = compare_models(
        model1=fno_model,
        model2=fnodynamquant_model,
        test_loaders=fnotest_loaders,
        data_processor=fnodata_processor,
        device=device,
        track_performance=True
    )
    results_by_model["fno"][bits] = fnocompare
    
generate_graph(results_by_model, quantisation_bits, "dynamic_quantisation", "No of bits", "Bits", savefile="compression/quantization/evaluation/all_dynamic_quant_performance.png")
